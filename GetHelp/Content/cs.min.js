...
return chainable?elems:bulk?fn.call(elems):length?fn(elems[0],key):emptyGet;};var rcheckableType=(/^(?:checkbox|radio)$/i);(function(){var input=document.createElement("input"),div=document.createElement("div"),fragment=document.createDocumentFragment();div.innerHTML="  <link/><table></table><a href='http://app.comagic.ru/a'>a</a><input type='checkbox'/>";support.leadingWhitespace=div.firstChild.nodeType===3;support.tbody=!div.getElementsByTagName("tbody").length;support.htmlSerialize=!!div.getElementsByTagName("link").length;support.html5Clone=document.createElement("nav").cloneNode(true).outerHTML!=="<:nav></:nav>";input.type="checkbox";input.checked=true;fragment.appendChild(input);support.appendChecked=input.checked;div.innerHTML="<textarea>x</textarea>";support.noCloneChecked=!!div.cloneNode(true).lastChild.defaultValue;fragment.appendChild(div);div.innerHTML="<input type='radio' checked='checked' name='t'/>";support.checkClone=div.cloneNode(true).cloneNode(true).lastChild.checked;support.noCloneEvent=true;if(div.attachEvent){div.attachEvent("onclick",function(){support.noCloneEvent=false;});div.cloneNode(true).click();}
...
(function(){var div,style,a,pixelPositionVal,boxSizingReliableVal,reliableHiddenOffsetsVal,reliableMarginRightVal;div=document.createElement("div");div.innerHTML="  <link/><table></table><a href='http://app.comagic.ru/a'>a</a><input type='checkbox'/>";a=div.getElementsByTagName("a")[0];style=a&&a.style;if(!style){return;}
...
return jQuery;}));(function(g){if(!g.uLiB){g.uLiB={}}
...
maskEl.remove();el.removeClass(this.getCssSelector(this.C.CSS_ELEMENT_MASKED,true));},zoomElement:function(el){var zoom='normal';if(navigator.userAgent.indexOf('Windows Phone')===-1){zoom=window.innerWidth<window.innerHeight?window.innerWidth/480:window.innerHeight/640;}
el=el?this.ensure$(el):_$('body');el.css({zoom:zoom});},zoomElement2:function(el){this.ensure$(el).css('zoom',window.innerWidth/document.documentElement.clientWidth);},fixElementOnBottom:function(el){var zoom=window.innerWidth<window.innerHeight?window.innerWidth/480:window.innerHeight/640,bottom=Math.floor((document.body.clientHeight-window.innerHeight-window.pageYOffset)/zoom),left=Math.floor(window.pageXOffset/zoom),right=Math.floor((document.body.clientWidth-window.innerWidth-window.pageXOffset)/zoom);el.css({zoom:zoom,bottom:bottom,left:left,right:right});},getCssSelector:function(selector,autoPrefix,isClassSelector){selector=selector.replace(/_/g,'-');selector=selector.replace(/\s{2,}/g,' ');var parts=selector.split(' '),ret=[],part;while(part=parts.shift()){if(u.arrayIsContain(['>','*'],part)){ret.push(part);continue;}
...
return Math.max(0,Math.max.apply(null,_$.map(a,function(v){var zIndex=parseFloat(_$(v).css('z-index'));return zIndex===2147483647||!zIndex?null:zIndex;})));},zIndexFilterFn:function(el){return el.style&&el.style.position!=='static';},comagicTopZIndex:function(){var zIndex=this.topZIndex('*',this.zIndexFilterFn);return zIndex>10000?zIndex:10000;},getZIndex:function(offset){offset=offset||0;return this.C.BASE_ZINDEX+(+offset);},getZIndexStr:function(offset){return''+this.getZIndex(offset);},addStyle:function(styles,id){id=id||(new Date()).getTime();var tpl='<style type="text/css" id="{prefix}{id}">{styles}</style>',opts={prefix:'comagic-dynamic-style-',id:id,styles:styles},stylesEl=_$('#'+id),stylesHtml=this.sformat(tpl,opts);return stylesEl.length?stylesEl.replace(stylesHtml):_$(stylesHtml).appendTo('head');},removeStyle:function(id){_$('#'+id).remove();},addCssLink:function(o){if(o.widget_color){o=u.apply({banner_color:o.widget_color},o);}
var cssLink=o.css_url?o.css_url:this.sformat(this.C.TPL_CSS_LINK,this.apply({},{comagic_web_url:this.ensureEndsWith(o.comagic_web_url,'/')},o)),debugCssUrl=this.getDebugParam('cssurl'),cssSkin=cssLink.split('-')[0].split('/').pop(),cssId='comagic-banner-stylesheet-'+cssSkin,cssElem=_$('#'+cssId);if(debugCssUrl){if(this.endsWith(debugCssUrl,'.css')){cssLink=debugCssUrl+'?'+Math.floor((Math.random()*100)+1);}else{cssLink=this.sformat(this.C.TPL_CSS_LINK_DEBUG,this.apply({url:debugCssUrl},o));}}
if(cssElem.length&&!o.iframe){cssElem.attr('href',cssLink);}else{_$('<link/>').attr('rel','stylesheet').attr('id',cssId).attr('type','text/css').attr('href',cssLink).appendTo(o.iframe?o.iframe.find('head'):'head')}},removeCssSkinLink:function(o){var cssLink=o.css_url?o.css_url:this.sformat(this.C.TPL_CSS_LINK,o),cssSkin=cssLink.split('-')[0].split('/').pop(),cssId='comagic-banner-stylesheet-'+cssSkin,cssElem=_$('#'+cssId);cssElem.length&&cssElem.remove();},addSitePhoneCssLink:function(o){var sitephoneCssLink=this.sformat(this.C.TPL_SITEPHONE_CSS_LINK,o),cssId='comagic-sitephone-stylesheet',cssElem=_$('#'+cssId);if(cssElem.length){cssElem.attr('href',sitephoneCssLink);}else{_$('<link/>').attr('rel','stylesheet').attr('id',cssId).attr('type','text/css').attr('href',sitephoneCssLink).appendTo(o.iframe?o.iframe.find('head'):'head')}},jfind:function(rootEl,selector){return rootEl.find(this.getCssSelector(selector,true,true));},jhasClass:function(el,cls){return el.hasClass(this.getCssSelector(cls,true));},camelize:function(str){return str.replace(/(?:^|[-_])(\w)/g,function(_,c){return c?c.toUpperCase():'';})},sformat:function(tpl,params){var s=arguments[0],o=arguments[1],regex;if(_$.type(o)=='object'){var field,fields={},value;regex=/{([^}]+)}/g;while(field=regex.exec(s)){field=field[1];value=o[field];if(value===undefined){continue;}
...
if(scope===void 0&&u._$.type(obj)==='object'){scope=obj;}
if(u._$.type(fn)==='array'){args=fn;}
if(u._$.type(scope)==='array'){args=scope;}
if(u._$.type(fn)==='function'){return fn.apply(scope||this,args||[]);}
return null;},getDeviceType:function(){if(this.C.IS_PHONE){return'Phone';}
if(this.C.IS_TABLET){return'Tablet';}
return'PC';},haveToDisplayLabelOnDevice:function(visibilities){return!visibilities||visibilities.indexOf(this.getDeviceType())!==-1;},init:function(){this.__debugparams=this.getURLParameter('bannerdebug');this.__debug=(!__cs||__cs.__debug===undefined)?!!this.__debugparams:__cs.__debug;this.__debugger=this.getURLParameter('bannerdebug')==2;try{this.__debugparams=JSON.parse(this.__debugparams);}catch(e){this.log('Cannot parse debug params. Going to plain debug mode.')}
this.C.BASE_ZINDEX=this.comagicTopZIndex()+1;var timeDelta=this.getURLParameter('time_delta');timeDelta!==''&&this.dSetTimeDelta(timeDelta);},getChunks:function(str,maxLength){var _str=str||'',tmpAr=_str.split(' '),result=[''];for(var i=0;i<tmpAr.length;i++){var lastItem=result[result.length-1];if(!lastItem.length||((lastItem.length+tmpAr[i].length)<maxLength-1)){result[result.length-1]=[lastItem,tmpAr[i]].join(' ').trim();}else{result.push(tmpAr[i]);}}
return result;},formatPhone:function(phone,cityLength,phoneTpl,subPhoneMask){subPhoneMask=subPhoneMask||'###-##-##';phoneTpl=phoneTpl||'+{country} ({city}) {phone}';var subPhone=phone.substr(cityLength+1),subPhoneMaskOctothorpCount=(subPhoneMask.match(/#/g)||[]).length,subPhoneLength=subPhone.length,count=0;if(subPhoneMaskOctothorpCount>subPhoneLength){for(var i=0;i<subPhoneMaskOctothorpCount-subPhoneLength;i++){subPhoneMask=subPhoneMask.replace('#','');}}else if(subPhoneMaskOctothorpCount<subPhoneLength){for(var i=0;i<subPhoneLength-subPhoneMaskOctothorpCount;i++){subPhoneMask='#'+subPhoneMask;}}
subPhone=subPhoneMask.replace(/#/g,function(){return subPhone[count++]||'';});return this.sformat(phoneTpl,{country:phone.substr(0,1),city:phone.substr(1,cityLength),phone:subPhone});}};C.IS_TABLET=C.IS_TOUCH_DEVICE&&!C.IS_PHONE;u.apply(u,{C:C});u.init();u.apply(u.ns(namespace,g),u);})(window,'http://app.comagic.ru/static/uLiB.util');(function(g){var _globals=['Comagic','__cs'],_queue=[];function isArray(obj){return Object.prototype.toString.call(obj)==='[object Array]';}
function isNumber(obj){return typeof obj==='number';}
function isString(obj){return typeof obj==='string';}
function maybeCall(obj,method,scope,args){var fn=obj[method];if(typeof fn!=='function'){return;}
fn.apply(scope||obj,args);}
for(var i=0,gName;(gName=_globals[i]);i++){if(g[gName]&&isArray(g[gName])){_queue=_queue.concat(g[gName]);}}
if(!g.uLiB){g.uLiB={}}
if(!g.uLiB.util){g.uLiB.util={};}
g.uLiB.util.Queue={length:_queue.length,push:function(c){return _queue.push(c);},shift:function(index){if(index===void 0){return _queue.shift();}
if(isNumber(index)){return _queue.splice(index,1)[0];}
if(isString(index)){for(var i=0,action;(action=_queue[i]);i++){if(isArray(action)&&action[0]===index){return this.shift(i);}}}},peek:function(index){if(index===void 0){return _queue[0];}
if(isNumber(index)){return _queue[index];}
if(isString(index)){for(var i=0,action;(action=_queue[i]);i++){if(isArray(action)&&action[0]===index){return action;}}}
return null;},_process:function(item,obj,scope){if(!item){return null;}
var method,args;method=item.shift();args=item.length===1&&isArray(item[0])?item[0]:item;return maybeCall(obj,method,scope||obj,args);},process:function(obj,scope){var qi;while(qi=_queue.shift()){this._process(qi,obj,scope);}},processCommands:function(commands,obj,scope){if(!this.length){return;}
for(var i=0,c;(c=commands[i]);i++){this._process(this.shift(c),obj,scope);}}};})(window);if(!window.console){window.console={};}
if(!window.console.log){window.console.log=function(){};}
try{window.__cs_onReplacePhones=window.__cs_onReplacePhones||function(phones){return true;};window.Comagic=window.__cs=$.extend(new function(){if(!window.uLiB){window.uLiB={};}
this.$=window.uLiB.$=window.jQuery.noConflict(true);this.push=function(param){uLiB.util.Queue.push(param);};var cm_$=this.$;var SiteSettings={_namesOfScripts:['cs.min.js'/*tpa=http://app.comagic.ru/static/cs.min.js*/,'comagic.min.js'/*tpa=http://app.comagic.ru/static/comagic.min.js*/,'comagic.debug.js'/*tpa=http://app.comagic.ru/static/comagic.debug.js*/],_scriptNameRe:/[^\/]+.js/,_hostRe:/\/\/([^\/]+)/,_webHost:null,_siteKey:null,_urlTpl:'//{0}{1}',_urls:{settings:'/ss/settings/',consultant:'comagic/consultant.{0}.js'/*tpa=http://app.comagic.ru/static/comagic/consultant.{0}.js*/,sitephone:'comagic/sitephone.{0}.js'/*tpa=http://app.comagic.ru/static/comagic/sitephone.{0}.js*/,sitephonemobile:'comagic/sitephonemobile.{0}.js'/*tpa=http://app.comagic.ru/static/comagic/sitephonemobile.{0}.js*/},_bundles:{'consultant':'consultant.js'/*tpa=http://app.comagic.ru/static/consultant.js*/,'sitephone':'sitephone.js'/*tpa=http://app.comagic.ru/static/sitephone.js*/,'sitephonemobile':'sitephonemobile.js'/*tpa=http://app.comagic.ru/static/sitephonemobile.js*/},_sformat:function(format){var args=Array.prototype.slice.call(arguments,1);return format.replace(/{(\d+)}/g,function(match,number){return typeof args[number]!='undefined'?args[number]:match;});},_getSessionStorageParam:function(itemName,fieldName){var ss=uLiB.util.sessionStorage();if(!ss){return null;}
...
var scripts=document.getElementsByTagName('script'),src,fileMatch,hostMatch;for(var i=0,s;(s=scripts[i]);i++){src=s.getAttribute('src');fileMatch=this._scriptNameRe.exec(src);if(fileMatch&&this._namesOfScripts.indexOf(fileMatch[0])!==-1){hostMatch=this._hostRe.exec(src);return hostMatch&&hostMatch[1];}}
return null;},_composeUrl:function(mnemonic,type){if(uLiB.util.C&&uLiB.util.C.IS_PHONE){mnemonic={'sitephone':'sitephonemobile'}[mnemonic]||mnemonic}
var webHost={script:this._getSessionStorageParam('comagic-widget:settings','script_host')}[type]||this.getWebHost();return this._sformat(this._urlTpl,webHost,mnemonic&&this._sformat(this._urls[mnemonic],(this._siteSettings||{}).script_debug?'debug':'min')||'');},_getSiteKey:function(){return this._siteKey;},_onSiteSettingsLoad:function(r){var me=this;if(!r||!r.status===200||!r.responseJSON||!r.responseJSON.success){console.log('SiteSettings request failed',r);return;}
var d=this._siteSettings=r.responseJSON.data;if('consultant'in d){this._loadScript(this._composeUrl('consultant','script'));}
if('sitephone'in d){this._loadScript(this._composeUrl('sitephone','script'),function(){uLiB.sitephone.Sitephone.init(cm_$.extend({site_key:me._getSiteKey(),comagic_web_url:me._composeUrl(),is_consultant_enabled:'consultant'in d},d['sitephone']));});}},_loadScript:function(url,callback){var script=document.createElement('script');script.src=url;script.onload=callback;document.head.appendChild(script);},getWebHost:function(){return this._webHost||(this._webHost=this._getWebHost());},request:function(mnemonic,callback){var url=this._composeUrl(mnemonic);if(uLiB.util.IE.version<=9&&window.XDomainRequest){uLiB.util.IE.xrequest(url+'?sk='+this._getSiteKey(),callback);}else{cm_$.ajax({url:url,crossDomain:true,data:{sk:this._getSiteKey()},complete:callback});}},init:function(){this._siteKey=uLiB.util.Queue.peek('setAccount')[1];SiteSettings.request('settings',this._onSiteSettingsLoad.bind(this));}};SiteSettings.init();},window.Comagic,window.__cs);}catch(e){console.log(e);}
(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function Observable(o){this.initObservable();}
u.ensureFunctionName(Observable);Observable.prototype.initObservable=function(){this.events={};};Observable.prototype.on=function(eventName,callback,scope){var listeners=this.events[eventName]||[];listeners.push({fn:callback,scope:scope});this.events[eventName]=listeners;};Observable.prototype.un=function(eventName,callback){var listeners=this.events[eventName]||[],tmp=[];for(var i=0,lr;(lr=listeners[i]);i++){if(lr.fn!==callback){tmp.push(lr);}}
this.events[eventName]=tmp;};Observable.prototype.fireEvent=function(eventName){var args=Array.prototype.slice.apply(arguments),listeners;eventName=args.shift();listeners=this.events[eventName]||[];for(var i=0,lr;(lr=listeners[i]);i++){lr.fn.apply(lr.scope||this,args);}};Observable.prototype.hasListeners=function(eventName){return!!(this.events[eventName]&&this.events[eventName].length);};u.apply(u.ns(ns),{Observable:Observable});})(window,'http://app.comagic.ru/static/uLiB.util');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function EventBus(o){EventBus.superclass.constructor.apply(this,arguments);}
u.extend(EventBus,'uLiB.util.Observable');u.apply(u.ns(ns),{EventBus:new EventBus()});})(window,'http://app.comagic.ru/static/uLiB.util');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$,$=_$;(function($){var pluses=/\+/g;function encode(s){return config.raw?s:encodeURIComponent(s);}
...
return false;break;case'get':if(typeof(Storage)!=="undefined"){var item=ls.getItem(key);if(item){try{item=JSON.parse(item);}catch(err){return item;}
if(!item.ezstorage){return item;}
if(item.expires&&new Date(item.expires)<new Date()){ls.removeItem(key);return null;}
return(options.full)?item:item.value;}
else{item=ss.getItem(key);if(item){try{return JSON.parse(item);}catch(err){return item;}}}}
return $.cookie(key);break;case'set':var v=JSON.stringify(value);if(typeof(Storage)!=="undefined"){if(options.expires||options.persist){v=JSON.stringify($.extend({},{value:value},options));ls.setItem(key,v);}
else{ss.setItem(key,v);}}
else{$.cookie(key,v,options)}
return v;break;case'remove':if(typeof(Storage)!=="undefined"){ls.removeItem(key);ss.removeItem(key);}
...
return this;};})(window,'uLiB.');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;_$.fn.anchor=function(target,anchor,monitorScrollAndResize,offset){var me=this;if(u._$.type(monitorScrollAndResize)==='object'){offset=monitorScrollAndResize;monitorScrollAndResize=false;}
if(!offset){offset={};}
if(!offset.x){offset.x=0;}
if(!offset.y){offset.y=0;}
function doAnchor(){if(u.C.IS_TOUCH_DEVICE){return;}
var parts=anchor.split('-'),_$target=_$(target),_$this=_$(me),targetPos=_$target.offset(),bodyOffset,bodyPosition=_$('body').css('position'),parentOffset,_$parent=_$this.parent(),parentPosition=_$parent.css('position'),x=0,y=0;if(parts[0].indexOf('b')!=-1){y+=_$target.outerHeight();}
if(parts[0].indexOf('r')!=-1){x+=_$target.outerWidth();}else if(parts[0].indexOf('c')!=-1){x+=(_$target.outerWidth()-_$this.outerWidth())/2;}
...
if(!_$parent.is('body')&&parentPosition!=='static'){parentOffset=_$parent.offset();y-=parentOffset.top;x-=parentOffset.left;}
y=y<0?0:y;x=x<0?0:x;_$(me).css({top:y+'px',left:x+'px'});}
...
if(this.options.fade){$tip.stop().css({opacity:0,display:'block',visibility:'visible',zIndex:u.getZIndexStr(10)}).animate({opacity:this.options.opacity});}else{$tip.css({visibility:'visible',opacity:this.options.opacity,zIndex:u.getZIndexStr(10)});}}},hide:function(){if(this.options.fade){this.tip().stop().fadeOut(function(){$(this).remove();});}else{this.tip().remove();}},fixTitle:function(){var $e=this.$element;if($e.attr('title')||typeof($e.attr('original-title'))!='string'){$e.attr('original-title',$e.attr('title')||'').removeAttr('title');}},getTitleCfg:function(){var title,$e=this.$element,o=this.options;this.fixTitle();var title,o=this.options;if(typeof o.title=='string'){title=$e.attr(o.title=='title'?'original-title':o.title);}else if(typeof o.title=='function'){title=o.title.call($e[0]);}
...
return l;};window.mina=mina;return mina;})(window.uLiB.eve);var Snap=(function(root){Snap.version="0.4.0";function Snap(w,h){if(w){if(w.nodeType){return wrap(w);}
...
$(p.node,{viewBox:[x,y,width,height].join(" "),markerWidth:width,markerHeight:height,orient:"auto",refX:refX||0,refY:refY||0,id:p.id});p.node.appendChild(this.node);return p;};function slice(from,to,f){return function(arr){var res=arr.slice(from,to);if(res.length==1){res=res[0];}
return f?f(res):res;};}
...
(s.rotate?"r"+[+s.rotate.toFixed(4),0,0]:E);}else{return"m"+[this.get(0),this.get(1),this.get(2),this.get(3),this.get(4),this.get(5)];}};})(Matrix.prototype);Snap.Matrix=Matrix;Snap.matrix=function(a,b,c,d,e,f){return new Matrix(a,b,c,d,e,f);};});Snap.plugin(function(Snap,Element,Paper,glob,Fragment){var has="hasOwnProperty",make=Snap._.make,wrap=Snap._.wrap,is=Snap.is,getSomeDefs=Snap._.getSomeDefs,reURLValue=/^url\(#?([^)]+)\)$/,$=Snap._.$,URL=Snap.url,Str=String,separator=Snap._.separator,E="";eve.on("http://app.comagic.ru/static/snap.util.attr.mask",function(value){if(value instanceof Element||value instanceof Fragment){eve.stop();if(value instanceof Fragment&&value.node.childNodes.length==1){value=value.node.firstChild;getSomeDefs(this).appendChild(value);value=wrap(value);}
if(value.type=="mask"){var mask=value;}else{mask=make("mask",getSomeDefs(this));mask.node.appendChild(value.node);}!mask.node.id&&$(mask.node,{id:mask.id});$(this.node,{mask:URL(mask.id)});}});(function(clipIt){eve.on("http://app.comagic.ru/static/snap.util.attr.clip",clipIt);eve.on("snap.util.attr.clip-path",clipIt);eve.on("snap.util.attr.clipPath",clipIt);}(function(value){if(value instanceof Element||value instanceof Fragment){eve.stop();if(value.type=="clipPath"){var clip=value;}else{clip=make("clipPath",getSomeDefs(this));clip.node.appendChild(value.node);!clip.node.id&&$(clip.node,{id:clip.id});}
...
eve.on("http://app.comagic.ru/static/snap.util.attr.fill",fillStroke("fill"));eve.on("snap.util.attr.stroke",fillStroke("stroke"));var gradrg=/^([lr])(?:\(([^)]*)\))?(.*)$/i;eve.on("snap.util.grad.parse",function parseGrad(string){string=Str(string);var tokens=string.match(gradrg);if(!tokens){return null;}
...
this.node.appendChild(txt);})(-1);eve.on("http://app.comagic.ru/static/snap.util.attr.path",function(value){eve.stop();this.attr({d:value});})(-1);eve.on("snap.util.attr.class",function(value){eve.stop();this.node.className.baseVal=value;})(-1);eve.on("snap.util.attr.viewBox",function(value){var vb;if(is(value,"object")&&"x"in value){vb=[value.x,value.y,value.width,value.height].join(" ");}else if(is(value,"array")){vb=value.join(" ");}else{vb=value;}
...
node.appendChild(tp);this.textPath=wrap(tp);}}}})(-1);eve.on("http://app.comagic.ru/static/snap.util.attr.text",function(value){if(this.type=="text"){var i=0,node=this.node,tuner=function(chunk){var out=$("tspan");if(is(chunk,"array")){for(var i=0;i<chunk.length;i++){out.appendChild(tuner(chunk[i]));}}else{out.appendChild(glob.doc.createTextNode(chunk));}
...
return out;}
eve.on("http://app.comagic.ru/static/snap.util.getattr.text",function(){if(this.type=="text"||this.type=="tspan"){eve.stop();var out=textExtract(this.node);return out.length==1?out[0]:out;}})(-1);eve.on("snap.util.getattr.#text",function(){return this.node.textContent;})(-1);eve.on("snap.util.getattr.viewBox",function(){eve.stop();var vb=$(this.node,"viewBox");if(vb){vb=vb.split(separator);return Snap._.box(+vb[0],+vb[1],+vb[2],+vb[3]);}else{return;}})(-1);eve.on("snap.util.getattr.points",function(){var p=$(this.node,"points");eve.stop();if(p){return p.split(separator);}else{return;}})(-1);eve.on("http://app.comagic.ru/static/snap.util.getattr.path",function(){var p=$(this.node,"d");eve.stop();return p;})(-1);eve.on("snap.util.getattr.class",function(){return this.node.className.baseVal;})(-1);function getFontSize(){eve.stop();return this.node.style.fontSize;}
...
return out;}
function isNumeric(obj){return isFinite(parseFloat(obj));}
function arrayEqual(arr1,arr2){if(!Snap.is(arr1,"array")||!Snap.is(arr2,"array")){return false;}
return arr1.toString()==arr2.toString();}
Element.prototype.equal=function(name,b){return eve("snap.util.equal",this,name,b).firstDefined();};eve.on("snap.util.equal",function(name,b){var A,B,a=Str(this.attr(name)||""),el=this;if(isNumeric(a)&&isNumeric(b)){return{from:parseFloat(a),to:parseFloat(b),f:getNumber};}
...
-$el.outerHeight(true)
-paddingV;newCoords.top=Math.min(newCoords.top,maxTop);newCoords.top=Math.max(newCoords.top,minTop);newCoords.left=Math.min(newCoords.left,maxLeft);newCoords.left=Math.max(newCoords.left,minLeft);return newCoords;}})(_$);})(window,'uLiB.');(function(g,ns){var u=g.uLiB.util,_$=u._$;if(_$.ionSound){return;}
...
var input=$(this),buffer=$.map(mask.split(""),function(c,i){return"?"!=c?defs[c]?getPlaceholder(i):c:void 0;}),defaultBuffer=buffer.join(""),focusText=input.val();input.data($.mask.dataName,function(){return $.map(buffer,function(c,i){return tests[i]&&c!=getPlaceholder(i)?c:null;}).join("");}),input.one("unmask",function(){input.off(".mask").removeData($.mask.dataName);}).on("http://app.comagic.ru/static/focus.mask",function(){if(!input.prop("readonly")){clearTimeout(caretTimeoutId);var pos;focusText=input.val(),pos=checkVal(),caretTimeoutId=setTimeout(function(){input.get(0)===document.activeElement&&(writeBuffer(),pos==mask.replace("?","").length?input.caret(0,pos):input.caret(pos));},10);}}).on("http://app.comagic.ru/static/blur.mask",blurEvent).on("http://app.comagic.ru/static/keydown.mask",keydownEvent).on("http://app.comagic.ru/static/keypress.mask",keypressEvent).on("http://app.comagic.ru/static/input.mask paste.mask",function(){input.prop("readonly")||setTimeout(function(){var pos=checkVal(!0);input.caret(pos),tryFireCompleted();},0);}),chrome&&android&&input.off("http://app.comagic.ru/static/input.mask").on("http://app.comagic.ru/static/input.mask",androidInputEvent),checkVal();});}});})(_$);})(window,'uLiB.jplugin');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function Stateful(o){}
...
if(renderBefore){el.insertBefore(o.renderBefore);}else if(renderTo){u.isInstance(renderTo,'uLiB.cmp.Component')?el.appendTo(renderTo.getEl()):el.appendTo(renderTo);}
return el;}
u.apply(u.ns(ns),{Element:Element});})(window,'http://app.comagic.ru/static/uLiB.el');(function(g,ns){var u=g.uLiB.util;function Div(o){o.tag='div';return Div.superclass.constructor.call(this,o);}
...
u.extend(TestDiv,Div);u.apply(u.ns(ns),{Div:Div,Footer:FooterEl,TestDiv:TestDiv});})(window,'http://app.comagic.ru/static/uLiB.el');(function(g,ns){var ul=g.uLiB,u=ul.util,Element=ul.el.Element,_$=u._$;function Label(o){this.tag='label';Label.superclass.constructor.apply(this,arguments);}
...
u.extend(TestLabel,Label);u.apply(u.ns(ns),{Label:Label,TestLabel:TestLabel});})(window,'http://app.comagic.ru/static/uLiB.el');(function(g,ns){var ul=g.uLiB,u=ul.util,Element=ul.el.Element,_$=u._$;function Table(o){this.tag='table';Table.superclass.constructor.apply(this,arguments);}
...
u.extend(TestTable,Table);u.apply(u.ns(ns),{Table:Table,TestTable:TestTable});})(window,'http://app.comagic.ru/static/uLiB.el');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function Form(o){}
...
this.initEvents&&this.initEvents();this.monitorResize===true&&this.delayUntilCssLoad(u.bind(this._adjustPosition,this));};Component.prototype.initEvents=function(){var events=CMP_EVENTS_TO_LISTEN_TO;if(this.trackClick&&this._onClick){this.getEl().on('click',u.wrapEvent(this.__eventNormalizer,this._onClick,this));}
...
return cls.split(/\s+/);};Component.prototype._setElClass=function(cls){this.el&&this.el.attr('class',(this.cls=cls));};Component.prototype._onResize=function(){this._adjustPosition();};Component.prototype.delayUntilCssLoad=function(f){var me=this,fn=f;if(this.getEl()&&this.getEl().css('opacity').substr(0,4)==="0.99"){fn();}else{setTimeout(function(){me.delayUntilCssLoad(fn)},150);}};Component.prototype._getWidth=function(){return u.elementWidth(this.getEl());};Component.prototype._getHeight=function(){return u.elementHeight(this.getEl());};Component.prototype._adjustPosition=function(){if(this.o.banner_skin==='touch'){return;}
if(this.is_preview){return;}
if(this.ctype==='label'&&this.hasCls('right')){return;}
if(this instanceof uLiB.cmp.ModalPanel){return;}
var me=this,cmpMargin=8,cmpMarginStr=u.sformat('{0}px',cmpMargin),ww=u.elementWidth(),wh=u.elementHeight(),cmpEl=me.getEl(),cmpw=me._getWidth(),cmph=me._getHeight(),railAxis=u.arrayIsContain((this.o.banner_place||'').split('_'),['bottom','top'])?'x':'y';if(!cmpEl){return;}
...
this.state=this.globalState||this.state;if(railAxis==='x'&&this.state&&this.state.moveToRightBorder){cmpEl.css('left',u.elementWidth()-cmpw-30);this.savePosition();return;}
if(railAxis==='y'&&this.state&&this.state.moveToBottomBorder){cmpEl.css('top',u.elementHeight()-cmph-10);this.savePosition();return;}
if(this.state&&this.state.css){if(u.arrayIsContain((this.o.banner_place||'').split('_'),['bottom','top'])){if(u.elementWidth()-this.state.css.left-cmpw<30){cmpEl.css('left',u.elementWidth()-cmpw-30);}}else{if(u.elementHeight()-this.state.css.top-cmpEl.height()<10){cmpEl.css('top',u.elementHeight()-cmph-10);}}}
if(u.arrayIsContain((this.o.banner_place||'').split('_'),['bottom','top'])){if(u.elementWidth()-cmpEl.offset().left-cmpw<50){this.globalState&&(this.globalState.moveToRightBorder=true);}}else{if(u.elementHeight()-cmpEl.offset().top-cmph<25){this.globalState&&(this.globalState.moveToBottomBorder=true);}}
this.savePosition();};Component.prototype.savePosition=function(){if(this.isVisible()){var railAxis=u.arrayIsContain((this.o.banner_place||'').split('_'),['bottom','top'])?'x':'y';this.state=this.state||{};this.state.css=this.state.css||{};if(this.respectBannerPlace){if(railAxis==='x'){this.state.css.left=parseInt(this.getEl().css('left'))||this.getEl().offset().left||this.globalState.css.left;}else{this.state.css.top=parseInt(this.getEl().css('top'))||this.getEl().offset().top||this.globalState.css.top;}}
this.globalState=u.apply(this.globalState,this.state);this.saveState();}};Component.prototype.restorePosition=function(){if(this.o.banner_skin==='touch'){return;}
this.restoreState();if(this.hasGlobalState){this.state.css=this.globalState.css;}
if(!this.o.is_preview&&this.getEl()&&this.state.css&&Object.keys(this.state.css).length){var railAxis=u.arrayIsContain((this.o.banner_place||'').split('_'),['bottom','top'])?'x':'y';if(railAxis==='x'){this.state.css.left&&this.hasCls('right')&&this.removeCls('right');if(u.elementWidth()<this.state.css.left+this._getWidth()){this.getEl().css('left',u.elementWidth()-this._getWidth()-30);}else{this.getEl().css('left',this.state.css.left);}}else{if(u.elementHeight()<this.state.css.top+this._getHeight()){this.getEl().css('top',u.elementHeight()-this._getHeight()-30);}else{this.getEl().css('top',this.state.css.top);}}}};Component.prototype.addCls=function(cls){cls=u.getCssSelector(cls,true);var parts=this._getClsParts();if(u.inArray(cls,parts)!=-1){return;}
...
this._setElClass(this.cls=parts.join(' '));};Component.prototype.toggleCls=function(cls){if(this.hasCls(cls)){this.removeCls(cls);return false;}
this.addCls(cls);return true;};Component.prototype.isVisible=function(){if(this.showing||this.hiding){return this.showing;}
...
this.getEl().dragon(this.getDraggableOpts());this.isDraggableInited=true;};Component.prototype.getDraggableOpts=function(){return{};};Component.prototype.getParentByCType=function(ctype){var parentEl=this.getEl().parents(u.sformat('[comagic-type={0}]',ctype)).get(0);if(parentEl){return ComponentManager.get(parentEl.id);}
return null;};Component.prototype.getContrastColor=function(color,invert){var invertColor=invert||false,notWhite={'#232426':'#FFFFFF','#CFCCCC':'#4D4C4C','#E6E9E9':'#555656','#7BCC78':'#2A4C29','#FAD331':'#574E07','#F8B200':'#564300','#BBA68A':'#463D32'};return notWhite[color]||(invertColor?'#000000':'#FFFFFF');};Component.prototype._onKeydown=function(){};u.apply(u.ns(ns),{ComponentManager:ComponentManager,Component:Component});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function Button(o){this.addCls('button');var action=this.action||o.action,icon=o.icon||this.icon;if(action){this.ctype=[this.ctype,action].join('-');}
...
u.extend(ToolButtonClose,ToolButton);function ToolButtonCloseWithChar(o){this.addCls('tool-button-close-char');this.action='close';this.html='<div>&times;</div>'
ToolButtonCloseWithChar.superclass.constructor.apply(this,arguments);}
u.extend(ToolButtonCloseWithChar,ToolButton);function ToolButtonUnpin(o){this.addCls('tool-button-unpin');this.action='unpin';ToolButtonUnpin.superclass.constructor.apply(this,arguments);}
...
u.extend(ToolButtonMinimize,ToolButton);function MsButton(o){this.addCls('ms-button');this.addCls('unselectable');MsButton.superclass.constructor.apply(this,arguments);}
...
u.extend(FunctionalButton,MsButton);FunctionalButton.prototype.ctype='functional-button';FunctionalButton.prototype.trackOwnEvents=true;function CallGeneratorMainButton(o){this.addCls('retention-main-button');CallGeneratorMainButton.superclass.constructor.apply(this,arguments);this.getEl().css('background-color',o.color);this.getTextEl().css('color',this.getContrastColor(o.color))}
u.extend(CallGeneratorMainButton,CommonButton);CallGeneratorMainButton.prototype.ctype='common-button-send';function LeadRequestFormButton(o){this.addCls('lead-request-form-send-button');LeadRequestFormButton.superclass.constructor.apply(this,arguments);}
u.extend(LeadRequestFormButton,CommonButton);LeadRequestFormButton.prototype.ctype='common-button-send';u.apply(u.ns(ns),{Button:Button,StandardButton:StandardButton,SmallButton:SmallButton,ToolButton:ToolButton,ToolButtonClose:ToolButtonClose,ToolButtonCloseWithChar:ToolButtonCloseWithChar,ToolButtonUnpin:ToolButtonUnpin,ToolButtonPin:ToolButtonPin,ToolButtonMinimize:ToolButtonMinimize,MsButton:MsButton,CommonButton:CommonButton,PanelToolButton:PanelToolButton,CallButton:CallButton,RatingButton:RatingButton,LinklikeButton:LinklikeButton,ChatSendButton:ChatSendButton,FunctionalButton:FunctionalButton,CallGeneratorMainButton:CallGeneratorMainButton,LeadRequestFormButton:LeadRequestFormButton});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,Component=ul.cmp.Component,msButtonClass=ul.cmp.MsButton,buttonClass=ul.cmp.Button,_$=u._$;function Menu(o){this.addCls('menu');this.autoEl={tag:'div'};this.zIndex=3;this.renderTo=o.o&&o.o['renderTo']?o.o['renderTo']:'body';this.trackClick=true;this.trackOwnEvents=true;Menu.superclass.constructor.apply(this,arguments);this.show(false);this.renderItems();}
...
return null;};u.apply(u.ns(ns),{Menu:Menu});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$,Div=ul.el.Div;function Panel(o){this.is_preview=o.is_preview;this.addCls('panel');this.addCls(this.ctype);Panel.superclass.constructor.apply(this,arguments);}
...
this[name]=tmpItems;this[containerElName].appendTo(this.getEl());};Panel.prototype._showModal=function(show){var targetEl=u.ensure$(this.targetEl),me=this;if(show){this.show(false);var maskEl=!this.is_preview&&u.maskElement(targetEl,this.isMaskPositionFixed,me.getEl());me.addCls('form-panel-fixed');me.getEl().center(targetEl.is('body')?undefined:this.is_preview?u.ensure$('body'):targetEl,'hv',true).css('z-index',u.comagicTopZIndex()+2);setTimeout(function(){me.getEl().show();},100)}else{this.getEl().hide();u.unmaskElement(targetEl);}};u.apply(u.ns(ns),{Panel:Panel});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,Button=ul.cmp.Button,_$=u._$;function FileUpload(o){this.addCls('file-upload');FileUpload.superclass.constructor.apply(this,arguments);var fileInput=this.getEl().find('input[type=file]');if(this.validExtensions){fileInput.attr('accept','.'+this.validExtensions.join(',.'));}else{fileInput.removeAttr('accept');}
...
u.extend(FileUpload,Button);FileUpload.prototype.ctype='file-upload';FileUpload.prototype.html='<a tabindex="0">{text}<input type="file" name="{id}" id="{id}-file-upload"/></a>';FileUpload.prototype.onSubmit=function(fileName){console.log('onSubmit',arguments)};FileUpload.prototype.onReceive=function(response){var resp=JSON.stringify(response);console.log('onReceive',resp,arguments)};FileUpload.prototype.onUnsupportedFileExtension=function(currentExtension,validExtensions){console.log('onUnsupportedFileExtension',arguments)};FileUpload.prototype.validExtensions=null;FileUpload.prototype.setUploadUrl=function(url){this.getEl().find('form').attr('action',url);};u.apply(u.ns(ns),{FileUpload:FileUpload});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var u=g.uLiB.util,Form=g.uLiB.form.Form,_$=u._$;function FormPanel(o){this.o=o;this.addCls('form-panel');FormPanel.superclass.constructor.apply(this,arguments);}
u.extend(FormPanel,'uLiB.cmp.Panel');u.mixIn(FormPanel,Form);FormPanel.prototype.destroyCssLink=function(){u.removeCssSkinLink(this.o);};u.apply(u.ns(ns),{FormPanel:FormPanel});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,FormPanel=ul.cmp.FormPanel,ToolButtonClose=ul.cmp.ToolButtonClose,_$=u._$;function ModalPanel(o){var targetEl=this.targetEl||(u.isInstance(o.target,'uLiB.cmp.Component')?o.target.getEl():o.target);targetEl=u.ensure$(targetEl);this.renderTo='body';this.trackClick=true;ModalPanel.superclass.constructor.apply(this,arguments);this.show(false);g.setTimeout(u.bind(function(){!o.is_preview&&this._showModal(true);},this),100);}
u.extend(ModalPanel,FormPanel);ModalPanel.prototype.respectBannerPlace=false;ModalPanel.prototype.ctype='modal-panel';ModalPanel.prototype.isModal=true;ModalPanel.prototype.tools=[ToolButtonClose];ModalPanel.prototype.onDestroy=function(){ModalPanel.superclass.onDestroy&&ModalPanel.superclass.onDestroy.apply(this,arguments);u.unmaskElement(this.targetEl);};u.apply(u.ns(ns),{ModalPanel:ModalPanel});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,Panel=ul.cmp.Panel,_$=u._$;function PopupMessagePanel(o){this.renderTo='body';if(o['banner_skin']=='flat'){this.addCls({info:'popup-message-panel-info',error:'popup-message-panel-error'}[o['type']]);}
...
u.extend(PhoneFieldValidator,RegexpFieldValidator);function MobilePhoneFieldValidator(){MobilePhoneFieldValidator.superclass.constructor.apply(this,arguments);this.name='mobilephone';this.noErrorMessage=true;this.regexp=/^\d{11}$/;}
u.extend(MobilePhoneFieldValidator,RegexpFieldValidator);function MaxLengthFieldValidator(o){this.name='maxlength';}
u.extend(MaxLengthFieldValidator,FieldValidator);MaxLengthFieldValidator.prototype.isValid=function(val,field){return val.length<=field.maxLength;};MaxLengthFieldValidator.prototype.errorMessage=function(val,field){return u.sformat('Максимальная длина: {0}',field.maxLength);};u.apply(u.ns(ns),{NotEmptyFieldValidator:NotEmptyFieldValidator,RegexpFieldValidator:RegexpFieldValidator,NumberFieldValidator:NumberFieldValidator,EmailFieldValidator:EmailFieldValidator,PhoneFieldValidator:PhoneFieldValidator,MaxLengthFieldValidator:MaxLengthFieldValidator,MobilePhoneFieldValidator:MobilePhoneFieldValidator});})(window,'uLiB.field');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$,Element=ul.el.Element,Component=ul.cmp.Component;function Field(o){o=o||{};this.addCls('field');this._validators=[];if(o.validators){this._validators=this._validators.concat(o.validators);}
...
if(this.placeholder){this.getInputEl().watermark(this.placeholder+this.getMarkRequired(),{className:u.getCssSelector('watermark',true)});}};Field.prototype.initEvents=function(){Field.superclass.initEvents.apply(this,arguments);var me=this,inputEl=this.getInputEl();if(!inputEl){return;}
inputEl.on('keyup',u.bind(this._doProcessValue,this));inputEl.on('keydown',u.bind(this._onKeydownCheckValue,this));inputEl.on('blur',u.bind(this._doProcessValue,this));if(this.tooltipType==='rich'){inputEl.on('focus',u.bind(this._onInputFocus,this));}};Field.prototype._onInputFocus=function(event){this.setError(false);};Field.prototype._onKeydownCheckValue=function(){var el=this.getEl(),inputEl=this.getInputEl();if(event.keyCode!==13){this._clearError(inputEl,el);}};Field.prototype._clearError=function(inputEl,el){inputEl.addClass(u.getCssSelector(this.fieldErrorNoMessageCls));inputEl.removeClass(u.getCssSelector(u.C.CSS_FIELD_ERROR,true));el.removeClass(u.getCssSelector(u.C.CSS_FIELD_CONTAINER_ERROR,true));};Field.prototype._doProcessValue=function(event){var me=this,inputEl=this.getInputEl(),fn=function(){var val=inputEl.val();if(me.lastErrorMessage===val){return;}
...
var el=this.getEl(),inputEl=this.getInputEl();if(message){validator.noErrorMessage&&inputEl.addClass(u.getCssSelector(this.fieldErrorNoMessageCls,true));inputEl.addClass(u.getCssSelector(u.C.CSS_FIELD_ERROR,true));el.addClass(u.getCssSelector(u.C.CSS_FIELD_CONTAINER_ERROR,true));}else{this._setTooltip();this._clearError(inputEl,el);}};Field.prototype._setRichTooltip=function(tooltipText,el){if(this.renderTooltipInside){var inputEl=this.getInputEl(),cls=u.getCssSelector(this.fieldInnerErrorCls,true);if(tooltipText){this._setRawValue(tooltipText);inputEl.addClass(cls);}else{this._setRawValue(this.value);inputEl.removeClass(cls);}}else{Field.superclass._setRichTooltip.apply(this,arguments);}};function TextField(o){this.autoEl={tag:'input',maxlength:1000};TextField.superclass.constructor.apply(this,arguments);}
...
u.extend(NumberField,TextField);function PhoneField(o){this.mask='+7 (999) 999-99-99';this.valueFilterCharsRegexp=/[^\d]/g;PhoneField.superclass.constructor.apply(this,arguments);!o.validators&&this.addValidator('phone');if(_$.type(_$.fn.mask)=='function'){this.getInputEl().mask(this.mask);}
this.getInputEl().attr('type','tel');}
u.extend(PhoneField,NumberField);_$.mask&&(_$.mask.autoclear=false);function CaptchaField(o){var me=this;this._captchaUrlProp='url';this._captchaKeyProp='key';this.allowBlank=false;this.addCls('captcha-field');this.tooltipGravity='nw';this.tooltipOptions={offsetH:-41};CaptchaField.superclass.constructor.apply(this,arguments);this.getInputEl().attr('type','number');}
u.extend(CaptchaField,TextField);CaptchaField.prototype.autoRefresh=false;CaptchaField.prototype.hasRefreshButton=false;CaptchaField.prototype.imagePosition='top';CaptchaField.prototype.extraFieldWrapperCls='captcha-field-wrapper';CaptchaField.prototype.render=function(){CaptchaField.superclass.render.apply(this,arguments);this.imgEl=new Element({tag:'img',cls:'captcha-field-image'});this.wrapperEl[this.imagePosition==='top'?'prepend':'append'](this.imgEl);if(this.hasRefreshButton){this.refreshEl=new Element({tag:'div',cls:'captcha-field-refresh-button'});this.refreshEl.click(u.bind(this.onRefreshButtonClick,this));this.wrapperEl.prepend(this.refreshEl);}
this.autoRefresh&&this.refreshCaptcha();};CaptchaField.prototype.getCaptchaProvider=function(){var me=this;return this['is_preview']?{get_captcha:function(callback){var ret={};ret[me._captchaUrlProp]='../../ksv.dev.ulab.ru/captcha.jpg'/*tpa=http://ksv.dev.ulab.ru/captcha.jpg*/;ret[me._captchaKeyProp]='SDAFdf3f2fqw3fw3efdsAFSDAf23';callback(ret);}}:window.Comagic.Captcha;};CaptchaField.prototype.refreshCaptcha=function(){this.setValue(Field.prototype.emptyValue);this.getCaptchaProvider().get_captcha(u.bind(this._onCaptcha,this),this.getCaptchaParams());};CaptchaField.prototype._onCaptcha=function(resp){this._setCaptchaUrl(resp[this._captchaUrlProp]);this.captchaKey=resp[this._captchaKeyProp];};CaptchaField.prototype._setCaptchaUrl=function(url){this.imgEl.attr('src',url);};CaptchaField.prototype.getValidationValue=function(){return CaptchaField.superclass.getValue.apply(this);};CaptchaField.prototype.getValue=function(){return{captcha_key:this.captchaKey,captcha_val:CaptchaField.superclass.getValue.apply(this)}};CaptchaField.prototype.onRefreshButtonClick=function(event){this.refreshCaptcha();};CaptchaField.prototype.getCaptchaParams=function(){if(this.captchaBackgroundColor){return{bg:this.captchaBackgroundColor};}
return{};};function TextAreaField(o){this.autoEl={tag:'textarea',maxlength:1000};TextAreaField.superclass.constructor.apply(this,arguments);}
...
if(!this.slimscrollApplied||force){this.getInputEl().slimscroll({height:height||'auto'});this.slimscrollApplied=true;}};u.apply(u.ns(ns),{Field:Field,TextField:TextField,NumberField:NumberField,PhoneField:PhoneField,CaptchaField:CaptchaField,TextAreaField:TextAreaField});})(window,'uLiB.field');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function PopupMessagePanel(o){g.setTimeout(u.bind(this.onTtlExpired,this),this.ttl);this.addCls('popup-message-panel');o.cls&&this.addCls(o.cls);var code=o.code,textObj=u.apply({},this.codeToText[code]),codeType=code.charAt(0);u.apply(this,textObj);this.processCode(code);if(textObj.tpl){textObj.text=this.processTpl();}
this.text=textObj.text||'';this.zIndex=u.comagicTopZIndex();this.addCls(this.codeTypeToCls[codeType]);this.respectBannerPlace=false;PopupMessagePanel.superclass.constructor.apply(this,arguments);this.show(true);if(u.C.IS_TOUCH_DEVICE){var me=this;this.popupMessageIntervalID=setInterval(function(){if(me.ownerComponent.opts.banner_place.split('_').indexOf('top')===-1){me.getEl().css('top',parseInt(me.ownerComponent.getEl().css('top'),10)-me.getEl().height()-me.ownerComponentPadding||me.ownerComponent.getEl().offset().top-me.getEl().height()-me.ownerComponentPadding);}else{me.getEl().css('top',parseInt(me.ownerComponent.getEl().css('top'),10)+me.ownerComponent.getEl().height()+me.ownerComponentPadding||me.ownerComponent.getEl().offset().top+me.ownerComponent.getEl().height()+me.ownerComponentPadding);}
me.getEl().css('left',parseInt(me.ownerComponent.getEl().css('left'),10)||me.ownerComponent.getEl().offset().left);},100);}}
u.extend(PopupMessagePanel,'uLiB.cmp.Component');PopupMessagePanel.prototype.ctype='popup-message-panel';PopupMessagePanel.prototype.renderHidden=true;PopupMessagePanel.prototype.ownerComponentPadding=10;PopupMessagePanel.prototype.animationEnabled=true;PopupMessagePanel.prototype.animationKey='PopupMessagePanel';PopupMessagePanel.prototype.html=''+'<div>'+'<div><h1>{title}</h1><p>{text}</p></div>'+'</div>';PopupMessagePanel.prototype.ttl=2000;PopupMessagePanel.prototype.codeTypeToCls={'0':'popup-message-panel-success','1':'popup-message-panel-failure','9':'popup-message-panel-system'};PopupMessagePanel.prototype.codeToText={'010':{title:'Чат отправлен',tpl:'на почту {email}'},'110':{title:'Отправка не удалась'},'020':{title:'Ожидайте звонка',text:'Мы позвоним в течение минуты'},'121':{title:'Неверно введен код'},'122':{title:'Система не отвечает',text:'В данный момент звонок невозможен'},'124':{title:'Система не отвечает',text:'Повторите попытку через минуту'},'030':{title:'Заявка принята',text:'Спасибо за обращение'}};PopupMessagePanel.prototype.respectBannerPlace=true;PopupMessagePanel.prototype.onTtlExpired=function(){var me=this;clearInterval(this.popupMessageIntervalID);this.show(false,{callback:function(){me.ownerComponent.showToolButtons&&me.ownerComponent.showToolButtons(true);me.afterClose&&me.afterClose(me);me.destroy();}});};PopupMessagePanel.prototype.processTpl=function(){return(this.text=u.sformat(this.tpl,this));};PopupMessagePanel.prototype.processCode=function(code){if(code==='010'){this.email=ul.cmp.ComponentManager.getBy(ul.banner.form.MainConsultantPanel).getEmailToSendChatTo();}};PopupMessagePanel.prototype.show=function(show){this.calculateShowStrategy();if(this.showStrategy==='in-replace'){this.ownerComponent.show(false);}else{this.getEl().css({position:u.C.IS_TOUCH_DEVICE?this.ownerComponent.getEl().css('position'):'absolute'});u.removeCls(this.getEl(),['top','bottom','left','right']);var ocp=this.ownerComponentPadding;this.getEl().anchor(this.ownerComponent.getEl(),this.showStrategy==='above'?'tc-bc':'bc-tc',{y:this.showStrategy==='above'?(this.ownerComponent.isLeadForm?-10:15):ocp});}
...
this.showStrategy=bannerPlace.indexOf('top')!=-1?'below':'above';};u.apply(u.ns(ns),{PopupMessagePanel:PopupMessagePanel});})(window,'http://app.comagic.ru/static/uLiB.banner.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;var PopupMessageManager={show:function(cmp,response,afterClose,o){return new ul.banner.cmp.PopupMessagePanel(u.apply({code:response.code,ownerComponent:cmp,renderTo:'body',afterClose:function(popupPanel){if(popupPanel.ownerComponent.deferredClearOnSubmit){popupPanel.ownerComponent.clearValues();}
if(popupPanel.ownerComponent.setSendBtnDisabled){popupPanel.ownerComponent.setSendBtnDisabled(false);}
if(afterClose){afterClose(popupPanel.ownerComponent,popupPanel);}}},o||cmp.o));},getCallback:function(cmp,callback){var me=this;return function(o){if(o.success){cmp.hasBeenSubmitted=true;}
me.show(cmp,o.result,function(){callback(o,cmp,me);})}}};u.apply(u.ns(ns),{PopupMessageManager:PopupMessageManager});})(window,'http://app.comagic.ru/static/uLiB.cmp');(function(g,ns){var ul=g.uLiB,u=ul.util,Element=ul.el.Element,Component=ul.cmp.Component,Form=ul.form.Form,PanelToolButton=ul.cmp.PanelToolButton,CommonButton=ul.cmp.CommonButton,_$=u._$;function CommonFormPanel(o){this.addCls('common-form-panel');o.is_preview&&this.addCls('preview');o.scenario_id&&this.addCls('leadgen-preview');this.zIndex=1;CommonFormPanel.superclass.constructor.apply(this,arguments);}
...
if(u.elementHeight()-event.clientY>this.resizedFormMargin){this.setHeight(newHeight+1);}}};u.apply(u.ns(ns),{CommonFormPanel:CommonFormPanel});})(window,'http://app.comagic.ru/static/uLiB.banner.form');(function(g,ns){var ul=g.uLiB,u=ul.util,CommonFormPanel=ul.banner.form.CommonFormPanel,PhoneField=ul.field.PhoneField,CaptchaField=ul.field.CaptchaField,LinklikeButton=ul.cmp.LinklikeButton,_$=u._$;function SitePhonePanel(o){this.addCls('site-phone');this.tools=['close'];this.title='Заказ звонка';this.buttonText='Перезвоните мне';this.items=[{__class:PhoneField,name:'phone',placeholder:'Ваш телефон',renderTooltipInside:true,dontClear:true,notemptyText:'Укажите телефон',allowBlank:false}];o['is_captcha_enabled']&&this.items.push({__class:CaptchaField,name:'captcha_val',label:'Введите код с изображения',allowBlank:false,is_preview:o['is_preview']},{__class:LinklikeButton,action:'refresh-captcha',text:'Обновить код'});SitePhonePanel.superclass.constructor.apply(this,arguments);this.captchaField=o['is_captcha_enabled']?this.getItem(CaptchaField):null;}
u.extend(SitePhonePanel,CommonFormPanel);SitePhonePanel.prototype.ctype=u.C.CW_SITE_PHONE;SitePhonePanel.prototype.ftype='site-phone';SitePhonePanel.prototype.nonCaptchaCls='non-captcha';SitePhonePanel.prototype.clearOnSubmit=true;SitePhonePanel.prototype.draggable=true;SitePhonePanel.prototype.stateful=true;SitePhonePanel.prototype.stateId='site-phone';SitePhonePanel.prototype.hasGlobalState=true;SitePhonePanel.prototype.refreshCaptcha=function(){this.o.is_captcha_enabled&&this.captchaField.refreshCaptcha();};SitePhonePanel.prototype.show=function(show){this.getChildEl(this.containerCls)[this.o.is_captcha_enabled?'removeClass':'addClass'](this.nonCaptchaCls);show&&this.refreshCaptcha();SitePhonePanel.superclass.show.apply(this,arguments);};u.apply(u.ns(ns),{SitePhonePanel:SitePhonePanel});})(window,'http://app.comagic.ru/static/uLiB.banner.form');(function(g,ns){var ul=g.uLiB,u=ul.util,_$=u._$;function Lead(o){this.formDefaults={};u.apply(this,o);this.forms={};this.hidedFormIds=[];this.bindBusHandlers();}
Lead.prototype={initTimers:function(timers){this.timers={};if(!timers){return;}
for(var leadScenarioId in timers){if(timers.hasOwnProperty(leadScenarioId)){this.timers[leadScenarioId]=g.setTimeout(u.bind(this.onTimerUp,this,[leadScenarioId]),timers[leadScenarioId]*1000);}}},initActions:function(actions){actions&&u.arrayForEach(actions,this.doAction,this);},refreshTimers:function(timers){this.cleanupTimers();this.initTimers(timers);},cleanupTimers:function(){for(var leadScenarioId in this.timers){if(this.timers.hasOwnProperty(leadScenarioId)){g.clearTimeout(this.timers[leadScenarioId]);}}},onTimerUp:function(leadScenarioId){g.Comagic.onLeadTimerUp(leadScenarioId);},bindBusHandlers:function(){this.eventBus=uLiB.util.EventBus;if(!this.eventBus){return;}
this.eventBus.on('rpc:init_lead',this.onRpcInitLead,this);this.eventBus.on('rpc:refresh_timers',this.onRpcRefreshTimers,this);this.eventBus.on('rpc:show_lead_form',this.onRpcShowLeadForm,this);this.eventBus.on('rpc:hide_lead_form',this.onRpcHideLeadForm,this);this.eventBus.on('ulib:event',this.onUlibEvent,this);},onRpcInitLead:function(server,params){this.registerForms();this.initTimers(params.timers);this.initActions(params.actions);},onRpcRefreshTimers:function(server,params){this.refreshTimers(params.timers);},onRpcShowLeadForm:function(server,params){this.doAction(params);},onRpcHideLeadForm:function(server,params){this.hideForm(params);},doAction:function(action,index,actions){if(action.action_type==='auto_invite'){return;}
action.settings.is_show_on_close?this.trackMouseForShowForm(action,true):this.showForm(action,true)},showForm:function(params){var me=this;if(!this.formDefaults.is_preview&&!g.Comagic.isReady()){return;}
this.registerForms();params.settings.log_id=params.log_id;var form=this.getForm(params);!form.isModal&&setTimeout(function(){me.formDefaults.is_preview?form.getEl().show():form.show(true);},150);return form;},getForm:function(params){var form=this.forms[params.log_id];if(!form){form=this.forms[params.log_id]=this.makeForm(params);}
return form;},makeForm:function(params){this.registerForms();return ul.cmp.ComponentManager.create(u.apply({ctype:params.action_type,is_lead_gen_preview:this.is_lead_gen_preview,is_preview:this.is_lead_gen_preview,eventBus:this.eventBus,comagic_web_url:this.comagic_web_url||this.getComagicWebUrl()}),u.apply(params.settings,this.formDefaults));},registerForms:function(){if(this.formsRegistered){return;}
ul.cmp.ComponentManager.registerCType(ul.banner.form.PersonalPanel);ul.cmp.ComponentManager.registerCType(ul.banner.form.OfflineMessageForm);ul.cmp.ComponentManager.registerCType(ul.banner.form.CallGeneratorPanel);this.formsRegistered=true;},hideForm:function(params){var currentCmp=this.forms[params.log_id];this.hidedFormIds.push(params.log_id);if(!currentCmp||currentCmp.hasBeenSubmitted){return;}
this.closeLeadForm(currentCmp);},trackMouseForShowForm:function(params){if(!g.Comagic.isReady()){return;}
var me=this,lastDate=null,lastY=null,lastV=null,isFormnShown=false,aMin=350;var onMouseMove=function(e){if(isFormnShown){u._$('body').unbind('mousemove',onMouseMove);return;}
...
var moveLength=Math.abs(y-lastY),v=moveLength/dateDelta,vDelta=v-lastV,a=vDelta/dateDelta;if(a*1000>aMin&&['lead','invite','in_chat'].indexOf(g.Comagic.getOCState())===-1){me.onClosePageAttemptDetected(params);isFormnShown=true;}
lastDate=date;lastY=y;lastV=v;};u._$('body').bind('mousemove',onMouseMove);},onClosePageAttemptDetected:function(params){if(this.hidedFormIds.indexOf(params.log_id)===-1){this.showForm(params);this.trackFormShowOnServer(params);}},trackFormShowOnServer:function(params){g.Comagic.trackLeadShow({action_type:params.action_type,action_log_id:params.log_id});},_startCall:function(formCmp){if(formCmp.isValid()){g.Comagic.onLeadFormSubmit(formCmp.s['log_id'],formCmp.getValues(),this.getOnFormSubmitCallback(formCmp.s['log_id']));}},onUlibEvent:function(cmp,deepCType,ctype,$event,target,isTopmostCmp){var formCtype=deepCType.split('--')[0],formCmp=cmp.getParentByCType(formCtype);if(!formCmp||!formCmp.isLeadForm||formCmp.is_preview){return;}
if(deepCType==='call_generator--field'&&$event.type==='keydown'){if($event.keyCode==13){this._startCall(formCmp);}}
if(ctype==='common-button-send'){this._startCall(formCmp);}else if(ctype==='tool-button-close'){if(formCmp.is_preview){return;}
this.closeLeadForm(formCmp);if(!formCmp.hasBeenSubmitted){g.Comagic.onLeadFormCancel(formCmp.s['log_id']);}}},getOnFormSubmitCallback:function(action_log_id){var me=this,form=me.getForm({log_id:action_log_id});if(form.getCallback){return form.getCallback();}else{return uLiB.cmp.PopupMessageManager.getCallback(form,me.onPopupMessageClose);}},onPopupMessageClose:function(result,cmp,popupPanel){if(cmp._showModal){cmp._showModal(false);}
cmp.destroy();},closeLeadForm:function(cmp){if(cmp.isModal){cmp.destroy();}else{cmp.show(false);}},getComagicWebUrl:function(){return ComagicBanner.prototype.instance.o.comagic_web_url;}};u.apply(u.ns(ns),{Lead:Lead});})(window,'http://app.comagic.ru/static/uLiB.lead');